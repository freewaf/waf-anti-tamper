...
static void md5_process_block(const void *buffer, size_t len, struct md5_ctx *ctx)
{
    ...
    #define CYCLIC(w, s) (w = (w << s) | (w >> (32 - s)))
        ...
        /* Before we start, one word to the strange constants.
         ...
         perl -e 'foreach(1..64){msg_printf "0x%08x\n", int (4294967296 * abs (sin $_))}'
         */
...
inline void *md5_value_align (void const *ptr, int alignment)
{
    char const *p0 = ptr;
    char const *p1 = p0 + alignment - 1;
    
    return (void *)(p1 - (int) p1 % alignment);
}

int md5_generate_value(char *filename, unsigned char *md5_value)
{
    FILE *fp;
    int err;
